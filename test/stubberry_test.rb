...
class StubberryTest < ActiveSupport::TestCase
  ...
  test "stub_must should raise an error whenever method NOT called inside a block" do
    assert_raise(StandardError) { stub_must(:to_s, true) {} }

    assert_nothing_raised { stub_must(:to_s, true) { to_s } }
  end

  test "stub_must_not should raise an error whenever method called inside a block" do
    assert_raise(StandardError) { stub_must_not(:to_s, true) { to_s } }
    assert_nothing_raised { stub_must_not(:to_s, true) {} }
  end
  ...
  test "stub_must_all should raise whenever any of stubbed method wasnt called" do
    assert_raise(StandardError) do
      stub_must_all({ to_s: true, is_a?: false }) do
        to_s
      end
    end

    begin
      stub_must_all({ to_s: true, is_a?: false }) { to_s }
    rescue => e
      assert_equal(e.to_s, "is_a? wasn't called")
    end

    begin
      stub_must_all({ to_s: true, is_a?: false }) { is_a?(String) }
    rescue => e
  ...
  test "stub/must_if_def methods yields if method not defined" do
    ...
    assert_equal(flow, [:stub_if_def, :stub_must_if_def])
  end
